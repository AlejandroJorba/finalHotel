...
<body class="flex flex-col bg-gray-300 w-screen h-screen font-sans overflow-x-hidden">
    ...
    <div class="flex justify-center mt-2">
        <p class="px-2 text-center text-xl border-b-2 border-gray-600 text-gray-900">Registro de empleados</p> 
    </div>     
    <form class="flex flex-row mx-auto justify-center items-center h-screen" action="SvModificarEmpleadoOk" method="get">
            
            <%
                Usuario u = (Usuario) request.getSession().getAttribute("modificarEmpleado");
                    Empleado em = u.getUsuarioEmpleado();
                    String nombre = em.getNombre();
                    String apellido = em.getApellido();
                    String cargo = em.getCargo();
                    int dni = em.getDni();
                    String direccion = em.getDireccion();
                    String usuario = u.getUsuario();
                    String contraseña = u.getPassword();
                    
                    
            %>
            <div class="grid grid-cols-2 gap-x-4 gap-y-4 bg-white p-4 rounded-3xl shadow-2xl">
                <p>DNI<input value="<%=dni%>" name="dni" type="text" class="border-2 rounded border-gray-400 w-full focus:outline-none focus:border-green-500 focus:ring ring-green-500" type="text" required></p>
                <p>Nombre<input value="<%=nombre%>" name="nombre" type="text" class= "border-2 rounded border-gray-400 w-full focus:outline-none focus:border-green-500 focus:ring ring-green-500" required></p>
                <p>Apellido<input value="<%=apellido%>" name="apellido" type="text" class= "border-2 rounded border-gray-400 w-full focus:outline-none focus:border-green-500 focus:ring ring-green-500" required></p>
                <p>Dirección<input value="<%=direccion%>" name="direccion" type="text" class="border-2 rounded border-gray-400 w-full focus:outline-none focus:border-green-500 focus:ring ring-green-500" required></p>
                <p>Cargo<select value="<%=cargo%>" name="cargo" class="border-2 rounded border-gray-400 w-full focus:outline-none focus:border-green-500 focus:ring ring-green-500" required>
                ...
                <p>Usuario<input value="<%=usuario%>" name="user" type="text" class="border-2 rounded border-gray-400 w-full focus:outline-none focus:border-green-500 focus:ring ring-green-500" required></p>
                <p>Contraseña<input value="<%=contraseña%>" name="password" type="password" class="border-2 rounded border-gray-400 w-full focus:outline-none focus:border-green-500 focus:ring ring-green-500" required></p>
