...
<body class="flex flex-col bg-gray-300 w-screen h-screen font-sans overflow-x-hidden">
    ...
    <div class="flex justify-center mt-2 mb-2">
        <p class="px-2 text-center text-xl border-b-2 border-gray-600 text-gray-900">Modificación de huéspedes</p> 
    </div>

        <form class="flex flex-row mx-auto justify-center items-center h-screen" action="SvModificarOk" method="get">
       <%
                Reserva res = (Reserva) request.getSession().getAttribute("reserva");
                    int personas = res.getCant_personas();
                    Date check_in = res.getCheck_in();
                    Date check_out = res.getCheck_out();
                    Usuario u = res.getUsuario();
                    String nombreUsu = u.getUsuario();
                    Huesped hues = res.getHuesped();
                    int dni = hues.getDni();
                    String nombreHues = hues.getNombre();
                    String apellidoHues = hues.getApellido();
                    String direccionHues = hues.getDireccion();
                    String profesionHues = hues.getProfesion();
                    Date nacimientoHues = hues.getNacimiento();
                    Habitacion hab = res.getHabitacion();
                    int num_hab = hab.getNum_habitacion();
        %>
            <div class="grid grid-cols-2 gap-x-4 gap-y-2 bg-white p-4 rounded-3xl shadow-2xl">
                <p>Cantidad de Personas<select value="<%=personas%>"name="cantidadHuesped" class="border-2 rounded border-gray-400 w-full focus:outline-none focus:border-green-500 focus:ring ring-green-500" required>
                ...
                <p>Nº de Habitación
                    <select value="<%=num_hab%>" name="numeroHabitacion" class="border-2 rounded border-gray-400 w-full focus:outline-none focus:border-green-500 focus:ring ring-green-500" required>
                    
                    <option value="<%=num_hab%>"><%=num_hab%></option>
                    <option value="0">-</option>
                     <%
                        Controladora control = new Controladora();
                        List<Habitacion> listhab = control.traerHabitaciones();
                        for (Habitacion h : listhab) {
                        
                        
                        %>   
                     <option value="<%=h.getNum_habitacion()%>"><%=h.getNum_habitacion()%></option>
                            <%}%>
                </select></p>
                <p>Check-in<input value="<%=check_in%>"name="check_in" type="date" class="border-2 rounded border-gray-400 w-full focus:outline-none focus:border-green-500 focus:ring ring-green-500" required></p>
                <p>Check-out<input value="<%=check_out%>" name="check_out" type="date" class="border-2 rounded border-gray-400 w-full focus:outline-none focus:border-green-500 focus:ring ring-green-500" required></p>
                <p>DNI<input value="<%=dni%>" name="dni" class="border-2 rounded border-gray-400 w-full focus:outline-none focus:border-green-500 focus:ring ring-green-500" type="text" required></p>
                <p>Nombre <input value="<%=nombreHues%>" name="nombre" type="text" class= "border-2 rounded border-gray-400 w-full focus:outline-none focus:border-green-500 focus:ring ring-green-500" required></p>
                <p>Apellido <input value="<%=apellidoHues%>" name="apellido" type="text" class= "border-2 rounded border-gray-400 w-full focus:outline-none focus:border-green-500 focus:ring ring-green-500" required></p>
                <p>Dirección <input value="<%=direccionHues%>" name="direccion" type="text" class= "border-2 rounded border-gray-400 w-full focus:outline-none focus:border-green-500 focus:ring ring-green-500" required></p>
                <p>Profesión <input value="<%=profesionHues%>" name="profesion" type="text" class= "border-2 rounded border-gray-400 w-full focus:outline-none focus:border-green-500 focus:ring ring-green-500" required></p>
                <p>Alta hecha por<input value="<%=nombreUsu%>" name="usuario" type="text" class= "pointer-events-none border-2 rounded border-gray-400 w-full focus:outline-none focus:border-green-500 focus:ring ring-green-500"></p>
                <input type="hidden" name="id" value="<%=res.getId_reserva()%>">
                <div class="col-span-2 mx-auto">
                    <button type="submit"class="w-48 hover:text-white font-bold bg-gray-400 text-gray-900 mt-2 px-2 rounded-3xl border-black hover:bg-green-500 transition duration-400">Modificar</button>
                </div>
            </div>
        </form>
