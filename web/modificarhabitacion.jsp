...
<body class="flex flex-col bg-gray-300 w-screen h-screen font-sans overflow-x-hidden">
    ...
    <!-- Formulario -->
        <div class="flex justify-center mt-2">
            <p class="px-2 text-center text-xl border-b-2 border-gray-600 text-gray-900">Registro de habitaciones</p> 
        </div>     
        <form class="flex flex-row mx-auto justify-center content-center h-screen" action="SvModificarHabitacionOk" method="get">
            <%
                Habitacion hab = (Habitacion) request.getSession().getAttribute("modificarHabitacion");
                    String nombre = hab.getNombre_habitacion();
                    int num = hab.getNum_habitacion();
                    int piso = hab.getPiso_habitacion();
                    double precio = hab.getPrecio_habitacion();
                    String tipo = hab.getTipo_habitacion();
                    long id_habitacion = hab.getId_habitacion();
            %>
            <div class="flex flex-col mt-4 mb-4 gap-x-4 gap-y-2 bg-white p-4 rounded-3xl shadow-2xl">
                <p>Piso <select value="<%=piso%>" name="pisoHabitacion" class="border-2 rounded border-gray-400 w-full focus:outline-none focus:border-green-500 focus:ring ring-green-500" required>
                ...
                <p>Nº de Habitación<input value="<%=num%>" name="numeroHabitacion" type="text" class="border-2 rounded border-gray-400 w-full focus:outline-none focus:border-green-500 focus:ring ring-green-500" required></p>
                <p>Nombre de habitación<input value="<%=nombre%>" name="nombreHabitacion" type="text" class="border-2 rounded border-gray-400 w-full focus:outline-none focus:border-green-500 focus:ring ring-green-500" required></p>
                <p>Tipo de Habitación<select value="<%=tipo%>" name="tipoHabitacion" class="border-2 rounded border-gray-400 w-full focus:outline-none focus:border-green-500 focus:ring ring-green-500" required>
                    <option value="<%=tipo%>"><%=tipo%></option>
                    <option value="No existente">-</option>
                    <option>Single</option>
                    <option>Doble</option>
                    <option>Triple</option>
                    <option>Múltiple</option>
                </select></p>
                <p>Precio por noche<input value="<%=precio%>" name="precioHabitacion" type="text" class="border-2 rounded border-gray-400 w-full focus:outline-none focus:border-green-500 focus:ring ring-green-500" required></p>
                <input type="hidden" name="idHabitacion" value="<%=id_habitacion%>">
