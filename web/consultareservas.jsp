...
<html lang="es">
    ...
    <body class="flex flex-col bg-gray-300 font-sans overflow-x-hidden">
        ...
        <!-- Botones consultas --->
            <div class="flex flex-row gap-10 bg-gray-800 text-gray-100 pt-2 justify-center">
                <form class="flex flex-col items-center bg-gray-900 rounded-3xl p-3" action="SvDesdeHasta" method="get">
                    <div><h1 class="border-b-2">Búsqueda por huésped desde/hasta</h1></div>
                    <div class="flex flex-col items-center mt-2">
                    <p>DNI</p><input class="text-black" type="text" name="dniConsulta">
                    <p>Desde</p><input class="text-black"  type="date" name="desde">
                    <p>Hasta</p><input class="text-black" type="date" name="hasta">
                    <p></p><button type="submit"class="w-48 hover:text-white font-bold bg-gray-400 text-gray-900 mt-2 px-2 rounded-3xl border-black hover:bg-green-500 transition duration-400">Buscar</button>
                    </div>
                </form>
                <form class="flex flex-col items-center bg-gray-900 rounded-3xl self-center p-4" action="SvConsultaUsuario" method="get">
                    <div><h1 class="border-b-2">Búsqueda por nombre de usuario</h1></div>
                    <div class="flex flex-col items-center mt-2">
                    <p>Usuario</p><input class="text-black" type="text" name="usuarioConsulta">
                    <button type="submit"class="w-48 hover:text-white font-bold bg-gray-400 text-gray-900 mt-2 px-2 rounded-3xl border-black hover:bg-green-500 transition duration-400">Buscar</button>
                    </div>
                </form>
                <form class="flex flex-col items-center bg-gray-900 rounded-3xl self-center p-4" action="SvConsultaDia" method="get">
                    <div><h1 class="border-b-2">Búsqueda por día de carga</h1></div>
                    <div class="flex flex-col items-center mt-2">
                    <p>Día de carga</p><input class="text-black" type="date" name="diaConsulta">
                    <button type="submit"class="w-48 hover:text-white font-bold bg-gray-400 text-gray-900 mt-2 px-2 rounded-3xl border-black hover:bg-green-500 transition duration-400">Buscar</button>
                    </div>
                </form>
            </div>
        ...
        <div class="flex items-start pt-10 justify-center min-h-screen bg-gray-800">
                ...
                <thead class="bg-gray-900 text-gray-400">
                    <tr>
                        <th class="p-3">Día de carga</th>
                        <th class="p-3 text-center">Personas</th>
                        <th class="p-3 text-center">Noches</th>
                        <th class="p-3 text-center">Check-in</th>
                        <th class="p-3 text-center">Check-out</th>
                        <th class="p-3 text-center">Nombre habitación</th>
                        <th class="p-3 text-center">Número habitación</th>
                        <th class="p-3 text-center">Dni</th>
                        <th class="p-3 text-center">Nombre</th>
                        <th class="p-3 text-center">Nacimiento</th>
                        <th class="p-3 text-center">Dirección</th>
                        <th class="p-3 text-center">Profesión</th>
                        <th class="p-3 text-center">Precio final</th>
                        ...
                        for (Reserva res : listaReserva) {
                            {
                                ...
                                int cant_personas = res.getCant_personas();
                                Date check_in = res.getCheck_in();
                                Date check_out = res.getCheck_out();
                                Date diaCarga = res.getDiaCarga();
                                int cant_noches = (int) res.getCant_noches();
                                double precio = res.getPrecio_final();
                                String precio_final = "$" + precio;
                                //Coloco los datos del huesped
                                Huesped huesped = res.getHuesped();
                                if (res.getHuesped() != null) {
                                    ...
                                    Date nacimiento = huesped.getNacimiento();
                                    String profesion = huesped.getProfesion();
                                    
                                    Usuario user = res.getUsuario();
                                    String username = user.getUsuario();
                                    Habitacion h = res.getHabitacion();
                                    int num = h.getNum_habitacion();
                                    String nombreHab = h.getNombre_habitacion();
                                    //Borrar y modificar
                                    long idReserva = res.getId_reserva();
                                    
                                    String nombreCompleto = nombre + " " + apellido;
                                    SimpleDateFormat formateo = new SimpleDateFormat("dd/MM/yyyy");
                                    String checkIn = formateo.format(check_in);
                                    String checkOut = formateo.format(check_out);
                                    String nacimientoFort = formateo.format(nacimiento);
                                    String dia = formateo.format(diaCarga);

                    %>
                    <tr class="bg-gray-900">
                        <td class="p-3 text-center">
                            <div class="ml-3">
                                <div class="text-gray-400"><%=dia%></div>
                            </div>
                        </td>
                        <td class="p-3 text-center"><%=cant_personas%></td>
                        <td class="p-3 text-center"><%=cant_noches%></td>
                        <td class="p-3 text-center"><%=checkIn%></td>
                        <td class="p-3 text-center"><%=checkOut%></td>
                        <td class="p-3 text-center"><%=nombreHab%></td>
                        <td class="p-3 text-center"><%=num%></td>
                        <td class="p-3 text-center"><%=dni%></td>
                        <td class="p-3 text-center"><%=nombreCompleto%></td>
                        <td class="p-3 text-center"><%=nacimientoFort%></td>
                        <td class="p-3 text-center"><%=direccion%></td>
                        <td class="p-3 text-center"><%=profesion%></td>
                        <td class="p-3 text-center"><%=precio_final%></td>
                        <td class="p-3 text-center"><%=username%></td>
                        <td class="p-3 text-center" >
                            <div class="flex">
                                <form class="text-gray-400 hover:text-red-500 mx-2" method="post" action="SvEliminar">
                                    <input type="hidden" name="idReserva" value="<%=idReserva%>">
                                    <button type="submit"><i class="fas fa-trash-alt"></i></button>
                                </form>
                                <form class="text-gray-400 hover:text-yellow-500 ml-2" method="post" action="SvModificar">
                                    <input type="hidden" name="idReserva" value="<%=idReserva%>">
    ...
    <style>
        ...
        tr td:nth-child(n+15),
        tr th:nth-child(n+15) {
