...
<html lang="es">
    ...
    <body class="flex flex-col bg-gray-300 font-sans overflow-x-hidden">
        <%
            ...
            Date diaConsulta = (Date) misesion.getAttribute("diaConsulta");
            SimpleDateFormat formateo = new SimpleDateFormat("dd/MM/yyyy");
            String diaConsultaFormat = formateo.format(diaConsulta);      
        %>
        <!-- Header -->
        <nav>
        ...
        <div class="bg-gray-800 text-gray-400 pt-6 pl-2">
            <h1>Estás consultando el día: <%=diaConsultaFormat%></h1>
        </div>
        <!-- Tabla -->
        <div class="flex items-start pt-10 justify-center min-h-screen bg-gray-800">
            <table class="table text-gray-100 border-separate space-y-6 text-sm p-3">
                ...
                <tbody>
                    ...
                    List <Reserva> listaReservas = (List) request.getSession().getAttribute("listaReserva");
                   for (Reserva res : listaReservas) {
                       Date dia = res.getDiaCarga();
                       if (diaConsulta.equals(dia)){
                        double noches = res.getCant_noches();
                        ...
                        String diaFormat = formateo.format(diaCarga); 
                        String nacimientoFormat = formateo.format(nacimiento); 
                        
                %>
                    <tr class="bg-gray-900">
                        <td class="p-3 text-center">
                                <div class="ml-3">
                                    <div class="text-gray-400"><%=diaFormat%></div>
                                </div>
                        </td>
                        <td class="p-3 text-center">
                            <%=personas%>
